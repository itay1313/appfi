...
export function Hero() {
  return (
    <section className="relative overflow-hidden bg-[#0c0a12]">
      {/* DotField fills the entire hero area */}
      <div className="absolute inset-0">
        <DotField
          ...
          bulgeStrength={50}
          glowRadius={180}
          gradientFrom="rgba(140, 80, 255, 0.3)"
          gradientTo="rgba(100, 70, 180, 0.15)"
          glowColor="#1a1525"
        />
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center px-6 pb-16 pt-20 text-center sm:pb-20 sm:pt-28">
        <div className="mb-6 rounded-2xl bg-white/10 p-3 shadow-2xl shadow-black/20 ring-1 ring-white/10 backdrop-blur-sm">
          {CHATGPT_ICON}
        </div>

        <h1 className="mb-3 max-w-lg text-3xl font-bold tracking-tight text-white sm:text-[2.5rem] sm:leading-tight">
          ChatGPT Reviews
        </h1>

        <p className="max-w-md text-[15px] leading-relaxed text-white/50">
          Explore what users are saying about the ChatGPT iOS app.
          Search, filter, and discover trends across thousands of reviews.
        </p>
      </div>
    </section>
  );
